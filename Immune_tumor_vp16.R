...
# Input files
curFilename = "VP16PPARd"
curTitle = "VP16PPARd vs Control"

seuratAll = Immune_VP16
...
## Plot clusters on UMAP embedding
{
  ...
  ggsave(file= 'VP16PPARd_Hull.pdf', width=2.5, height=2.5, units="in")
...
# Pathways
{
  ...
  for(curPathway in unique(pathwayScoresZscore$Pathway)){	
    ...
    sm3W$Diff = sm3W$VP16PPARd - sm3W$Control
    rangeDiff = max(abs(sm3W$Diff))
    
    ggplot(sm3W) + geom_tile(aes(x = x, y = y, fill = Diff)) + theme_cem + 
      scale_fill_gradientn(name = "VP16PPARd vs Control difference", colors = pathwayColorsDiff, limits = c(-rangeDiff, rangeDiff)) + 
    ...
    #ggsave(paste0("umap_densityDifference/", curFilename, "pathwayScoreZscore_umap_densityDifference_", curPathway, "_VP16PPARdVsControl.png"), width = dimredPlotWidth, height = dimredPlotHeight, dpi=150)
    #ggsave(paste0("umap_densityDifference/", curFilename, "pathwayScoreZscore_umap_densityDifference_", curPathway, "_VP16PPARdVsControl.pdf"), width = dimredPlotWidth, height = dimredPlotHeight)		
  }
  setwd(curPath)
...
{	
  ...
  ggsave(file= 'VP16PPARd_Pseudotime_umap_rainbow.pdf', width=2.5, height=2.5, units="in")
  ...
  ggsave("VP16PPARd_Pseudodensity_half_1.pdf", width = 1.6, height = 2, units="in")	
  ...
  ggplot(dimredP_half2, aes(x=Pseudotime, fill=Type)) + geom_density(alpha=0.4, bw=0.5, size = .1) + theme_cem + 
    ...
    scale_y_continuous(breaks = scales::breaks_extended(n = 3)) + 
    theme(strip.background = element_blank(), strip.text.y = element_blank(), legend.position = "none", text = element_text(size=5), axis.text.x = element_text(size =  6), axis.text.y = element_text(size = 6), axis.title.x = element_text(size = 7), axis.title.y = element_text(size = 7))
  ggsave("VP16PPARd_Pseudodensity_half_2.pdf", width = 1.6, height = 2, units="in")	
  ...
  ggsave( "VP16PPARd_frac_of_cells.pdf", width=4, height=2.5, units="in")
...
## Plot gene expression on dimred embedding
{
  ...
  geneList = unique(c('Ifi208', 'Gzmk',	'Gzma',	'Gzmb',	'Cxcr3', 'Cxcl10', 'Cxcl9',	'Ifng',	'Ifit1bl1',	'Bst2',	'Ifit1',	'Fos',	'Isg15',		'Ccl9',	'Irf7',	'Apobec1',	'Slfn1',	'Rps27',	'Bcl2',	'Ms4a4c',	'Ier5',	'Ly6c2',	'Itgb7',	'Ifi206')) # geneList3
  ...
  for(curGene in geneList)
  {
    ...
    sm3W = dcast(sm3, x+y~type, value.var="score")
    sm3W$VP16PPARd_vs_Control = sm3W$VP16PPARd - sm3W$Control
    rangeDiff = max(c(abs(sm3W$VP16PPARd_vs_Control)))
    
    ggplot(sm3W) + geom_tile(aes(x = x, y = y, fill = VP16PPARd_vs_Control)) +
      scale_fill_gradientn(name = "VP16PPARd vs Control", colors = geneColorsDiff, limits = c(-rangeDiff, rangeDiff)) +
...
for(i in cell_types){
  ...
  dimreda <- dimredaub %>% filter(Type == 'VP16PPARd')
  ...
  ggsave(paste0("VP16PPARd_pseudo_density_", cell, ".pdf"), width = 2.2, height = 1, units="in")	
}
...
ggsave(paste0("VP16PPARd_pseudo_density_Stem_1.pdf"), width = 2.2, height = 1, units="in")	
...
ggsave(paste0("VP16PPARd_pseudo_density_all_cell_type.pdf"), width = 3, height = 4, units="in")	
...

{
  ...
  ggsave(paste0("VP16PPARd_pseudo_density.pdf"), width = 2.2, height = 3.1, units="in")	
  ...
  ggsave(paste0("VP16PPARd_pseudo_density_all.pdf"), width = 2.2, height = 3.1, units="in")	
...
## Trajectory analysis
{
  ...
  model_VP16PPARd = model
  model <- readRDS('VP16PPARd_pseudotime.rds', refhook = NULL)
  ...
  identities <- levels(VP16PPARd.obj$Cell_type)
  ...
  #seuratExpSelVP16PPARd = seuratExpSel[, colnames(seuratExpSel) %in% dimred$Cell[ dimred$Type == "VP16PPARd"] ]
  ...
  pheatmap(seuratExpSelVP16PPARd, scale="none", cluster_rows = F, cluster_cols = F, show_colnames = F, breaks = breaks, treeheight_row = 0, treeheight_col = 0, color = colScale, annotation_col = annColumn, filename = paste0(curFilename, "_heatmapByPseudotime_byCells_VP16PPARd.png"), width=heatmapWidth, height=heatmapHeight, border_color = NA, main = "Heatmap of top 100 genes by pseudotime importance\nVP16PPARd Cells")
  pheatmap(seuratExpSelVP16PPARd, scale="none", cluster_rows = F, cluster_cols = F, show_colnames = F, breaks = breaks, treeheight_row = 0, treeheight_col = 0, color = colScale, annotation_col = annColumn,  filename = paste0(curFilename, "_heatmapByPseudotime_byCells_VP16PPARd.pdf"), width=heatmapWidth, height=heatmapHeight, border_color = NA, main = "Heatmap of top 100 genes by pseudotime importance\nVP16PPARd Cells")
  ...
  seuratExpSelMedSumW_Case = as.matrix(setRemoveRownames(dcast(seuratExpSelMedSum2[ seuratExpSelMedSum2$Type == "VP16PPARd", ], Gene~PseudotimeFactor, value.var="Expression")))
  ...
  pheatmap(seuratExpSelMedSumW_Case, scale="none", cluster_rows = F, cluster_cols = F, show_colnames = F, breaks = breaks, treeheight_row = 0, treeheight_col = 0, color = colScale, annotation_col = annColumn2, annotation_colors = annColors, filename = paste0(curFilename, "_heatmapByPseudotime_medianExpression_VP16PPARd.png"), width=heatmapWidth, height=heatmapHeight, border_color = NA, main = "Heatmap of top 100 genes by pseudotime importance\nMedian expression across pseudotime, VP16PPARd Cells")
  pheatmap(seuratExpSelMedSumW_Case, scale="none", cluster_rows = F, cluster_cols = F, show_colnames = F, breaks = breaks, treeheight_row = 0, treeheight_col = 0, color = colScale, annotation_col = annColumn2, annotation_colors = annColors, filename = paste0(curFilename, "_heatmapByPseudotime_medianExpression_VP16PPARd.pdf"), width=heatmapWidth, height=heatmapHeight, border_color = NA, main = "Heatmap of top 100 genes by pseudotime importance\nMedian expression across pseudotime, VP16PPARd")
  ...
  pheatmap(seuratExpSelMedSumW_Diff, scale="none", cluster_rows = F, cluster_cols = F, show_colnames = F, breaks = breaks, treeheight_row = 0, treeheight_col = 0, color = colScale, annotation_col = annColumn3, annotation_colors = annColors, filename = paste0(curFilename, "_heatmapByPseudotime_medianExpressionDifference_VP16PPARdvsCtrl.png"), width=heatmapWidth, height=heatmapHeight, border_color = NA, main = "Heatmap of top 100 genes by pseudotime importance\nVP16PPARd - Control median expression difference across pseudotime")
  pheatmap(seuratExpSelMedSumW_Diff, scale="none", cluster_rows = F, cluster_cols = F, show_colnames = F, breaks = breaks, treeheight_row = 0, treeheight_col = 0, color = colScale, annotation_col = annColumn3, annotation_colors = annColors, filename = paste0(curFilename, "_heatmapByPseudotime_medianExpressionDifference_VP16PPARdvsCtrl.pdf"), width=heatmapWidth, height=heatmapHeight, border_color = NA, main = "Heatmap of top 100 genes by pseudotime importance\nVP16PPARd - Control median expression difference across pseudotime")
